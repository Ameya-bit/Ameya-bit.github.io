...
**The rule: prose keeps Emil's voice, apparatus takes a second role.** Flat 16px→17px Inter, weight-only rank, instant hovers, no chrome — all unchanged. What is new is a **monospace role** for the things a paper has and a blog doesn't.

- **Split is strict.** Mono = labels and numerals (post filing line, section numbers, TOC, listing date + reading time, colophon). Inter = anything made of sentences (figure captions, descriptions, the epistemic block). Nothing with a verb in it goes mono.
- Tokens: `--font-apparatus` (system mono stack), `--text-apparatus-mono: 0.8125rem` (13.8px), `--text-label: 0.6875rem` (11.7px, uppercase, tracked — kickers only). Mono runs optically large, so it sits *below* `--text-apparatus`, not level with it. All mono carries `letter-spacing: -0.01em` to pull it back toward Inter's colour.
...
**Figure rhythm 64px** (was 32px) against a 26px paragraph gap — 2.5:1, so a figure reads as an event. Ciechanowski runs 3:1 and it is most of why his figures feel like the point of the page. 1.2:1 is very nearly uniform.
