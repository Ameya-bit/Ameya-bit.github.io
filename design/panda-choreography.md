...
*Started 2026-07-23. This is the choreography spec for the homepage hero panda troupe — the concept that supersedes "roaming troupe with a hat" (Candidate 6 in [whimsy.md](whimsy.md)). Base engine, palette, fencing rules, and build history live in [whimsy.md](whimsy.md) and [direction.md](direction.md); this doc specifies only what the troupe **does**. Status (2026-07-23): **all five phases shipped**, built directly into the site (`assets/pandas/pandas.js` + `styles.scss`), plus a Phase-3 observer refinement (navigation + scan). **Doubly historical as of 2026-08-05: this system was retired by design review on 2026-07-23 (see the banner above), and the successor that replaced it came off the homepage entirely on 2026-08-03** — so "today" and "live" throughout this doc mean the site as it stood in July 2026, not as it stands now. See **[What the troupe does today](#what-the-troupe-does-today-as-shipped)** just below for the behaviour as it was; per-phase as-built details and the build log follow. The durable facing/idle polish (finer sprite angles, drawn idle cels) is **parked permanently** — no AI art, and the cels can't be hand-drawn (see the Phase 3 heading).*

---

## What the troupe does today (as shipped)

A quick map of the live behaviour (all constants named live in `assets/pandas/pandas.js`; per-phase mechanics + the build log are further down).

1. **Entrance.** The stage starts empty. The **hat panda ambles in first, alone** (the protagonist establishes the tone), then the troupe walks on from the edges **a couple at a time** and starts wandering. Headcount is **viewport-aware** (`PANDA_COUNT` ≈ 20 wide / 12 mid / 7 narrow). ma5a's **tap-to-knock** and the **knock → fall → get-up** collision are kept for free roamers.

2. **Conga lines — form · grow · dissolve · reform.** Roamers periodically organise into **follow-the-leader snakes** (`manageLines()`): each follower steps into the cell the one ahead vacated, so a heading change at the front **ripples down the line** (the mechanic patching later leans on). Lines grow to **`LINE_CAP` (5)**; up to **`MAX_LINES` (2)** nucleate. A conga panda is an **unstoppable force** — it knocks free roamers aside without recoil and passes through other conga pandas. Lines **dissolve** on a jittered **max age** (`LINE_MAX_LIFE` 42s ± 12s), or sooner (**`POST_PATCH_LIFE` 8s**) once they've taken part in a patch; freed pandas roam and get re-recruited — the scene keeps breathing.

3. **The hat panda — the interpreter (the self-insert).** Fastest panda (`HAT_MOVE_MS`) and a **collision ghost** (never knocked, never knocks). It **oversees the lines from a distance** (`observe()`): stands off at `WATCH_NEAR`, only engages a **real conga (2+)**, and locks onto one line **15–30s** before rotating to another — not camping one team. It **navigates *around* the other pandas** (`stepWeaving()` + a crowd penalty in `bestAxis`) to reach an **uncrowded, clear-view** vantage (routing around the hero card), then **stands and faces the line, scanning it** — a small gaze shift along the line every ~2–4s (`pickGaze()`) over a breathing bob, on the settled **f1** idle pose. It keeps ma5a's calm **2s glide** (a snappier "darty" pass was tried and reverted — too fast).

4. **Activation patching — the intervention (~every 20–30s).** When **two lines each ≥ 3** long have headings **> 45° apart**, the hat panda intervenes (`runPatch()`): **freeze the whole troupe + spotlight** the two lines (dim the rest), stride to line B, **grab its tail panda, spin** (a true turn through the 8 facings), and **fling it on a parabolic arc** into line A's **upper-middle**, **swapping out** the resident there (shoved clear, sent roaming). On resume, line A's **tail peels onto B's heading as a new line** — the visible **bend is the patched activation propagating** downstream, while the clean front holds A's heading. **Not a knockout** (insert/displace, never ablate).

5. **Lifecycle loop.** The two split halves then **dissolve and reform** (Phase 5), so the scene cycles **form → two coexist → patch → split → dissolve → reform** with no pile-up and no incursion into the text zone.

**Fencing / perf (binding):** hero-only, `aria-hidden`, **clipped above the Writing cue**, the hero text block is a hard **exclusion zone** (pandas route around it), rAF/collision **paused off-screen and on a hidden tab**. **Reduced motion = a static tableau** (pandas placed at rest; no choreography).
...
- **Refinement — "stand out as the self-insert" (2026-07-23, code + existing cels only):** the observer no longer drifts through the troupe like a ghost; it **actively navigates around the other pandas** and **scans the line** while watching. Three code changes in `observe()`/`bestAxis` (no new art):
  - **Weaving traversal** — `stepWeaving()` replaces `stepToward()` in the relocate branch. Among the 8 grid steps toward the (card-routed) vantage it takes the one that best trades **progress vs. crowding** (`crowdAt()`, personal-space `AVOID_R = 85`, `WEAVE_CROWD_W = 0.7`), so it visibly threads between roamers and lines. It stays a collision ghost underneath, so this is only a preference — after `WEAVE_STUCK = 5` crowded ticks with no gain it ghosts straight through, so it always reaches the vantage (no deadlock). A mild-penalised "hold" (`WEAVE_HOLD_BIAS`) lets it briefly stand aside for a passing panda.
  - **Claims its own space** — `bestAxis()` now adds a crowd penalty (`AXIS_CROWD_W`), so it plants where it has a clear view **and** elbow room; if the troupe crowds its planted vantage past `CROWD_BUMP`, it quietly relocates to clearer air.
  - **Attentive scan (idle, no new cels)** — over the CSS breathing bob, a JS gaze shifts every ~2–4s (`GAZE_MIN`/`GAZE_MAX`) to a fresh point along the line (leader / middle / tail / a brief look-around, `pickGaze()`), snapping to the nearest of the 8 existing headings. Reads as studying the line; amplitude stays small so it never spins. This is the "scan" draft the parked plan recommended, done procedurally instead of with drawn cels. Idle also now pins **`ANIM.idle = [1]`** (the f1 legs-together settled pose) instead of the f0 contact stride, which read as mid-walk.
  - **Motion speed — kept the uniform 2s float.** A "darty-but-quiet" pass was tried (hat-only 0.5s sharp glide + `HAT_MOVE_MS` 540→440) and **reverted** (2026-07-23, Ameya: "he darts around too fast"). The hat keeps ma5a's `HAT_MOVE_MS = 540` and the shared 2s wrapper glide; it stands out by *behaviour* (navigation + scan + keeping its own space), not by moving faster than the troupe.

> **⚠ PARKED PERMANENTLY — the durable idle/facing polish needs *drawn* sprite cels, and Ameya has ruled out AI-generated art (2026-07-23: "I don't want a meaningful part of the art generated by AI," and can't hand-draw the gaps).** Sprite tooling was researched (PixelLab's "rotate character" → the other 7 directions from one reference cel; Aseprite + `willibrandon/pixel-plugin`; AI idle generators) — all AI-generation-first, so **rejected**. The behavioural "stand out" ask was instead met in **code + existing cels** (see the Refinement above). The two art-blocked items below stay parked unless the no-AI-art constraint changes:
