...
- **Refinement — "stand out as the self-insert" (2026-07-23, code + existing cels only):** the observer no longer drifts through the troupe like a ghost; it **actively navigates around the other pandas** and **scans the line** while watching. Three code changes in `observe()`/`bestAxis` (no new art):
  ...
  - **Darty-but-quiet motion (self-insert stands out)** — the hat panda deliberately breaks the troupe's uniform **2s float**: `.panda_wrapper.hat` glides in **0.5s with a sharp ease-out** (`cubic-bezier(0.22,0.9,0.24,1)`), and `HAT_MOVE_MS` 540→**440**. It's still planted and calm most of the time (15–30s line lock), but *when* it repositions it snaps cleanly rather than drifting — an **alert** observer, not a hyperactive one. The unsettling "slight suspicion" read comes from the contrast (still + too-precise) not from motion volume. Scan/focus durations are ms-normalised, so the quicker cadence speeds only traversal, not the scanning. Ameya: darty/reflexive, ok to break the 2s, "shouldn't be visibly weird" → quiet-observer tuning (2026-07-23).
