...
![Top ten heads by direct logit attribution to the IO−S direction, averaged over 100 prompts.](figures/fig1_dla_ranking.webp)
...
![Residual-stream patching over (layer, position), mean normalized recovery across 100 prompts. Two bright bands and nothing else.](figures/fig2_resid_patching.webp)
...
![Attention-head patching at the END position, layers 8 and up, mean recovery across 100 prompts. Outlined cells are the heads the IOI paper names.](figures/fig3_head_patching.webp)
...
![Path patching L8H6 into block 9, as a fraction of the full clean→corrupted swing, on the canonical prompt. Each bar narrows the receiver; only the bottom one is a single edge.](figures/fig5_path_patching.webp)
...
![Attention patterns for the two heads on the canonical prompt, same colour scale. The outlined cell in each panel is the END row's read. The BOS column is masked — see below.](figures/fig4_attention_patterns.webp)
...
<figure class="ioi-lens-figure">
  ...
  <noscript>
    <img src="figures/fig6_blind_spots.webp" alt="Every head in layers 8–11 scored by direct logit attribution against patching recovery at END. Four of the five named heads sit at the extremes of both measures; L8H6 sits alone, near zero on DLA and highest on patching.">
  </noscript>
  <figcaption>
